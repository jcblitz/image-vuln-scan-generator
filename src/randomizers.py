...
class VulnerabilityRandomizer:
    ...
    def randomize_vulnerabilities(self, vulnerabilities: List[Dict]) -> List[Dict]:
        ...
        for vuln in vulnerabilities:
            ...
            if "Severity" in randomized_vuln:
                randomized_vuln["Severity"] = self._generate_severity()
            
            # Randomize CVSS scores
            if "CVSS" in randomized_vuln and isinstance(randomized_vuln["CVSS"], dict):
                self._randomize_cvss_scores(randomized_vuln["CVSS"])
    ...
    def _generate_version(self) -> str:
        ...
        version_types = [
            lambda: f"{random.randint(0, 10)}.{random.randint(0, 20)}.{random.randint(0, 50)}",
            lambda: f"{random.randint(0, 5)}.{random.randint(0, 15)}",
            lambda: f"{random.randint(1, 3)}.{random.randint(0, 10)}.{random.randint(0, 20)}-r{random.randint(0, 10)}",
            lambda: f"{random.randint(2015, 2024)}{random.randint(1, 12):02d}{random.randint(1, 28):02d}",
        ]
    ...
    def _generate_cvss_score(self, version: str) -> float:
        """
        Generate realistic CVSS score based on version.
    ...
    def _randomize_cvss_scores(self, cvss_data: Dict[str, Any]) -> None:
        """
        Randomize CVSS scores in place.
        ...
        for source_key, source_data in cvss_data.items():
            if isinstance(source_data, dict):
                if "V2Score" in source_data:
                    source_data["V2Score"] = self._generate_cvss_score("V2")
                if "V3Score" in source_data:
                    source_data["V3Score"] = self._generate_cvss_score("V3")
